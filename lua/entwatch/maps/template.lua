return {
    {
        ...
        -- ENTWATCH_MODE_SPAM_PROTECTION_ONLY - тоже пустышка; материи по типу бензопилы
        ...
        -- ENTWATCH_MODE_LIMITED_USES - материя имеет одно или несколько использований за раз
        --                              если материя имеет КД, то выставляем параметр "cooldown"
        --                              так же для материй по типу Ультимы будет отображаться время, которое оно кастуется
        --                              если материя имеет несколько использований, то выставляем параметр "maxuses"
        -- ENTWATCH_MODE_COUNTER_FMIN_REACHED - материя работает через math_counter, обычно это огнемет или электро на fapescape
        --                                      у math_counter если три значения: m_OutValue, m_flMin и m_flMax;
        --                                      именно для этого режима, когда значение m_OutValue достигает m_flMin (обычно 0, если маппер не гандон), то материя перестаёт работать;
        --                                      так же можно менять значения через параметры "currentvalue", "hitmin" и "hitmax" (3-й параметр обычно не трогаем, он должен быть равен 2000, если мапеер не гандон)
        ...
        --                                      у math_counter если три значения: m_OutValue, m_flMin и m_flMax;
        --                                      именно для этого режима, когда значение m_OutValue достигает m_flMax, то материя перестаёт работать;
        --                                      так же можно менять значения через параметры "currentvalue", "hitmin" и "hitmax" (2-й параметр обычно не трогаем, он должен быть равен 0, если мапеер не гандон)
        ...
        -- КД или кол-во секунд, необходимое для того, чтобы скастовать материю (или сколько она действует) (mode == ENTWATCH_MODE_LIMITED_USES)
        ["cooldown"] = 15,
